...
제품 소유자가 지정한 기준에 따라 최초 AI NOTE 오픈소스를 제품 버전 `1.0`으로 정규화합니다. 이후 문서·병합·테스트 전용 커밋이 아니라 사용자에게 전달되는 기능 묶음마다 마이너 버전을 한 번 올립니다. 현재까지 기준판 이후 15회 업데이트되어 제품 버전은 `1.15`이며, 패키지의 Semantic Version은 `1.15.0`입니다.
