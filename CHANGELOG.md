...
- **운영자 관리**: 최고 운영자가 운영자 콘솔에서 일반 운영자를 해제(`DELETE /api/admin/operators/{id}`). 계정과 관리자 세션을 제거하고 감사 기록 `operator.revoked`를 남기며, 같은 이메일을 다시 초대할 수 있다. 최고 운영자 자신과 고객은 대상이 아니다.
- **수정**: 운영자 초대 링크가 요청이 들어온 호스트(터널 내부 http, SSH 포워딩 주소 등)로 만들어지던 것을 `APP_ORIGIN` 기준 공개 주소로 고정(`publicOrigin` 공용 모듈, 회의실 초대와 동일 규칙).
- **운영자 초대 메일**: 새 운영자 초대가 초대받는 사람의 메일로 설정 링크를 발송(`[Vision AI 미팅 에이전트] 운영자 초대`, 한국어). SMTP 미설정·발송 실패 시 링크 발급으로 자동 전환하고, 성공 시에도 콘솔에 수동 전달용 링크를 함께 표시. 임시 비밀번호 메일과 발송 경로를 `smtpMail` 공용 모듈로 통합.
- **통역 회의실 번역 품질**: 호흡으로 끊긴 전사 조각을 문장 단위로 합쳐 제출(`UtteranceCoalescer`: 종결 부호·화자/언어 전환·1.8초 대기·400자), live 번역 발행 뒤 직전 발화 4개를 문맥으로 넣은 재번역이 모든 상대 언어를 덮어씀. 번역 프롬프트에 사투리·구어체 표준화 지시 추가(ADR 0028 갱신 기록).
- **통역 음성 듣기**: 통역 회의실에서 상대 발화의 내 언어 번역을 이어폰으로 들을 수 있는 토글과 음성 선택(호스트·게스트 모두). 발화당 한 번, 켠 뒤의 대화만, 종료 시 자동 해제. 화면 문구 ja/en/zh 추가.
- **테스트**: 회의실·게스트 세션 테스트가 고정 날짜 fixture 때문에 시간이 지나면 깨지던 문제를 가짜 시계로 고정.
...
- **1.17.0 (2026-07-30) — 고객사 데모 준비**: 공급자 중립 UI(상호·모델명 비노출, `/live` 경로), 외부 요약 API 키 모드, 시스템 상태 2행 단순화, 사이드바 한국어 메뉴명, Noto Sans KR 폰트, 저장 재시도 멈춤·홈 새로고침 기아 등 안정성 수정.
- **1.16.0 (2026-07-30) — Global Meeting 언어 혼용·미팅 제어·자동 저장**: 코드 스위칭 판정, 1.5×·2.0× 음성 가속, 일시정지·이어서 진행, 종료 시 대화록·회의록 자동 저장.
- **1.15.6 (2026-07-30) — Global Meeting 대화록·음성 설정**: 음성·속도 선택, 하나의 연속 양언어 대화록, 동일 행 정렬, Speaker 표기와 Push-to-Talk 강조.
- **1.15.5 (2026-07-30) — Global Meeting Left Shift Push-to-Talk**: 제품 표시명 변경과 Space 대신 왼쪽 Shift를 사용하는 PTT 단축키 전환, 버튼 포커스·반복 입력 안전성 보완.
- **1.15.4 (2026-07-29) — 번역 fallback·TTS 오류 정리**: 한국어 완료 endpoint의 상대 언어 fallback과 동기 TTS speak 실패 session 정리.
