...
> 현재 회의·라이브러리·단어장·개인 설정은 승인된 고객 계정 ID를 해시한 서버 전용 테넌트 저장소로 분리됩니다. 다른 계정의 저장소를 전역 목록이나 회의 ID로 읽지 못하도록 모든 데이터 API와 데이터 페이지에서 같은 계정 경계를 적용합니다. 다만 파일 기반 단일 서버 구성은 고객 테스트용입니다. 상용 다중 조직 운영에서는 데이터베이스·객체 저장소·조직 권한·감사 로그까지 포함한 별도 운영 구조가 필요합니다.
