...
1. 최초 운영자 생성 API는 서버의 `127.0.0.1`/`localhost` 호스트에서만 열립니다. 공개 HTTPS 주소의 `/admin/setup`은 "서버 로컬에서만 열려요"를 표시하므로, 작업 PC에서 SSH 포트 포워딩으로 서버 loopback에 붙은 뒤 브라우저에서 엽니다.

   ```bash
   ssh -N -L 127.0.0.1:3300:127.0.0.1:3000 ubuntu@<서버 IP>
   ```

   포워딩이 열린 상태에서 `http://127.0.0.1:3300/admin/setup`을 열고 이름·이메일·비밀번호를 입력합니다. 화면에 한 번만 표시되는 인증 앱 QR 코드를 스캔하고(수동 입력용 설정 키 병기) 복구 코드를 암호 관리 도구에 보관한 뒤 포워딩을 닫습니다.
2. 운영자 계정은 긴 고유 비밀번호와 TOTP MFA를 사용합니다. 이후 로그인은 공개 HTTPS 주소의 `/admin/login`에서 합니다.
3. 추가 운영자(중간 관리자)는 최고 운영자가 `/admin`의 "새 운영자 초대"에서 발급합니다. 6절의 SMTP가 설정되어 있으면 초대받는 사람의 메일로 24시간 일회용 설정 링크가 발송되고, 미설정이거나 발송에 실패하면 콘솔에 링크가 표시되어 직접 전달합니다. 운영자는 고객 승인·결제·차단을 처리할 수 있고, 새 운영자 초대와 운영자 해제는 최고 운영자만 할 수 있습니다. 해제된 운영자는 즉시 로그아웃되며 같은 이메일로 다시 초대할 수 있습니다.
...
`.github/workflows/uptime.yml`이 15분마다 GitHub Actions에서 `TARGET_URL`(로그인 페이지)을 호출해 200이 아니면 실패로 끝납니다. 예약 실행이 실패하면 GitHub가 저장소 소유자에게 메일로 알립니다(Settings → Notifications의 Actions 알림이 켜져 있어야 함). 도메인을 바꾸면 워크플로의 `TARGET_URL`을 함께 바꿉니다. 수동 확인은 Actions 탭에서 `Uptime` → `Run workflow`입니다.
