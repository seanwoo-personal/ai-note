...
15. **공유 통역 회의실(1:1)** — 호스트가 회의실을 만들고 링크와 자동 생성 비밀번호로 게스트를 초대한다. 두 사람은 각자 자기 기기에서 같은 대화를 자기 언어 관점(내 발화는 상대에게 전달된 번역을 검수, 상대 발화는 내 언어 번역으로 이해)으로 본다. 음성은 화상회의나 같은 방이 담당하고 앱은 자막·번역 보조 화면이다. `화상회의`는 기기 출처, `같은 방`은 언어→화자 라벨→등록→수동 정정 순서로 화자를 귀속한다. 게스트 링크는 회의 종료 뒤 24시간 유효하며 그 안에 대화록과 회의록(언어 선택)을 내려받을 수 있다. 회의는 호스트 계정의 회의로 영구 보존된다(ADR 0028).
