...
- **목소리 등록(같은 방·호스트)**: 대화 목록 위 `목소리 등록` 패널에 좌석별 버튼(`{name} 목소리 등록` → 누르면 `{name} 님이 한 문장을 말해 주세요…` `aria-pressed` → 등록 뒤 `{name} 등록됨 · 다시 등록`). 등록 중에는 다른 버튼을 disabled하고 결과는 패널 안 polite status 한 줄로 알린다. 등록 문장도 대화에 그대로 기록된다.
...
- **통역 음성 듣기**: 말하기 버튼 아래 구분선 밑에 `통역 음성 듣기/끄기`(aria-pressed) · 음성 select(라벨 `통역 음성`) · polite status(켜짐: "이어폰을 끼면 상대 말이 끝난 뒤 내 언어로 들립니다.", 재생 중: "재생 중…", 실패: "통역 음성을 재생하지 못했습니다."). 게스트 화면에도 동일하게 있고, 회의 종료 화면에서는 사라진다.
- **발화 확정 타이밍**: 끝점마다 행이 생기지 않는다. 문장이 끝나거나 1.8초 이상 말이 멈추면 조각을 합쳐 한 행으로 올라오고, 상대 언어 번역은 live 문장이 먼저 보인 뒤 몇 초 안에 문맥을 반영한 문장으로 조용히 교체된다(깜빡임·강조 없음).
