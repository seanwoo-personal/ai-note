...
- **타이포그래피 전용 워드마크(최초 채택 → 2026-08-02 철회)** — 1차 출처가 제3자 로고 사용을 허가하지 않아 처음에는 로고를 넣지 않았다. 이후 **저장소 소유자가 고객사 로고 사용을 명시적으로 지시**해 공식 로고로 바꿨다. 권리 분석 자체는 바뀌지 않았고(약관은 등록상표·전 IP 유보), 사용 권한 보유 여부는 고객사를 대면하는 소유자에게 있다. 저장소 안에 서면 라이선스 근거는 없으며 그 사실을 숨기지 않는다.
- **로고 리컬러·트레이싱·재구성** — 변형은 원본 그대로 쓰는 것보다 상표 리스크가 크다. 기각. 공급된 SVG를 브랜드 컬러(`#0068b7`/`#e60012`) 그대로 렌더하며, 그래서 light/dark 양쪽에서 동일하게 보인다.
