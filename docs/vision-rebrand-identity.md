...
### 이 저장소의 준수 방식 (구현된 대로 — 2026-08-02 갱신)

- **공식 로고를 사용한다.** 헤더는 Vision 코퍼레이트 워드마크(`https://www.vision-net.co.jp/common/img/header_logo.svg`, 2026-08-02 취득)를 `public/brand/vision-logo.svg`로 로컬 서빙한다 — `src/components/BrandWordmark.tsx`, 문자열 단일 소스는 `src/lib/brand.ts`.
- 로고는 **공급된 그대로** 렌더한다. 리컬러·비율 왜곡·트레이싱·재구성을 하지 않는다(변형은 원본 사용보다 상표 리스크가 크다). 브랜드 컬러 `#0068b7`/`#e60012`를 그대로 두므로 light/dark 양쪽에서 동일하게 표시된다.
- **§4의 권리 분석은 그대로 유효하다.** 1차 출처는 제3자 로고 사용을 허가하지 않으며 약관은 등록상표와 전 IP를 유보한다. 이 저장소가 로고를 넣은 근거는 **저장소 소유자(고객사 대면 당사자)의 명시적 지시**이며, 사용 권한 보유 여부는 소유자에게 있다. 저장소 안에 서면 라이선스 근거는 없다 — PASS를 날조하지 않는다.
- 로고 사용을 되돌리려면 `BrandWordmark.tsx`를 타이포그래피 `BRAND_NAME` 렌더로 바꾸고 `public/brand/`를 지우면 된다. 그 외 브랜드 문자열·레이아웃은 영향받지 않는다.
