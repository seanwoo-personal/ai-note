...
function StatusPill({ phase, labels }: {
...
function TestProductTool({ workspaceId, folderId }: { workspaceId: string; folderId: string | null }) {
  const capture = useSonioxLiveCapture();
  const speech = useSonioxTts();
  return <TestProductMeetingPanel capture={capture} speech={speech} location={{ workspaceId, folderId }} />;
}
  ...
  return (
    <main id="main" className="w-full max-w-6xl space-y-7 px-4 py-10 sm:px-6 lg:px-8">
      <header>
        ...
        {(selection.tool === "transcription" || selection.tool === "test-product") && (
      ...
      {selection.tool === "test-product" && <TestProductTool workspaceId={selection.workspaceId} folderId={selection.folderId} />}
